...
> Last updated: 2026-09-02 · Latest commits: `88651cc` (image-editor rendering fixes: pencil visibility in the attachment tray, stroke coordinate alignment, crop-aspect = media aspect, selected-color highlight) on top of `c098bb9` (react-easy-crop CSS import), `8f6a4fa`, and all previous. **Current working tree:** media/file lightbox lifted from `MessageList` up to the `ChatPane` root so it covers the whole pane (header + message list + composer) — uncommitted. Branch `master` is 25 commits ahead of `origin/master`.
...
| Forwarded-message badge | ✅ New `is_forwarded` boolean column (`20260901_chat_messages_is_forwarded.sql`) set on every forwarded copy; **receiver-only** bubble badge (hidden on the sender's own bubbles): a small **"Reenviado"** label **with a `Forward` icon** above the content (`i18n` `forwarded` key, en/es) — always false/null for normal sends |
...
- `src/chat/components/ChatPane.tsx` — `replyTo` state, `handleSend` passes `reply_to_id`, flex root, mark-as-read on conversation open; message-list remount per conversation (`key`); **own the `MediaLightbox` state** and render it `absolute inset-0` at the pane root so the image/file preview covers header + message list + composer.
...
- `src/chat/spellcheck/SpellCheckNode.ts` — `TextNode` subclass (`spellcheck-text`) whose DOM carries the `chat-spell-error` class **and inline `borderBottom` style** (red line); plain-text+format transparent, so HTML output/state serialization are unaffected.
...
| **7 – Theming / Dark Mode / I18n** | Design tokens + en/es + per-user customizations | i18n layer (`en`/`es`, typed `TranslationKey`) fully wired incl. latest keys (`copyMessage`, `forward*`, `forwardSelected`, `forwardNone`, `forwardDone`, `editorCrop`, `editorTilt`, `editorDraw`, `language*`, `spell*`, `settings*`); **language selector UI in `ChatHeader`**; **composer spell-check (nspell + en/es dictionaries, squiggle + click-for-suggestions popover)**; **per-user chat customizations**: `chat_settings` table (per-user wallpaper + bubble colors) + `ChatSettingsService` + `useChatSettingsStore` (optimistic, persisted), **Chat Settings panel** (`Settings` button next to locale selector: WhatsApp-default wallpaper **or** uploaded image via new `/api/chat/wallpaper` + `chat-wallpapers` bucket, my/other bubble color swatches + reset), wallpaper applied to `ChatPane` background, bubble colors applied in `MessageList` | ◑ ~60% | Design-token system (`color-system.css`), full dark-mode token coverage, more customizations (user said "I'll add more later") |
...
|---------------------|-----------------------------------------------------|-------------------|-----------|
| ≈ 40 person-days | ≈ 15–20 person-days | Phases 0,1,2,3,4,6 (+ ~60% of 7) ≈ **~36–38 person-days** | Phases 5,9 (≈ 8–10 person-days) + Phase 10 + the 7/8 leftovers + deferred Phase 6 Lighthouse audit ≈ **~6–8 person-days** |
