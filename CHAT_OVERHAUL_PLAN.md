...
- **Phase 7 (≈100%)** — i18n layer (`en`/`es`, typed `TranslationKey`) fully wired with all keys (`moreActions`, `replyingTo`, `typingMulti`, `copyMessage`, `forward*`, `forwarded`, `editorCrop`, `editorTilt`, `editorDraw`, etc.).
...
- **ForwardModal + forwarded-message work (working tree, not committed):** forward preview shows Lexical HTML as natural language (`htmlToText` from `@/chat/utils`) while the sent copy keeps its HTML formatting; the modal's section divider borders (header/tabs/search/error/footer) removed; every forwarded copy is persisted with `is_forwarded=true` (new `20260901_chat_messages_is_forwarded.sql` migration; `ChatMessage`/`CreateMessageData` type fields; `chatService.sendMessage` insert) and the **receiver-only** bubble badge (hidden on your own sent bubbles) renders a small **"Reenviado"** label **with a `Forward` icon** above the content (`i18n` `forwarded` key, en/es).
- **Full-pane media/file lightbox (uncommitted):** `MediaLightbox` state lifted from `MessageList` to the `ChatPane` root (rendered `absolute inset-0 z-50` over the whole pane) so the image/file preview covers the header and composer too; `MessageList` now delegates tile taps via an `onOpenLightbox` prop.
...
### Next Move
1. Commit the full-pane media/file lightbox change; optionally push `master` (currently 25 commits ahead of `origin/master`).
