...
> Last updated: 2026-08-31 · Latest commits: `a3ace91` (Lexical whitespace fix), `23a747a` (REPLICA IDENTITY FULL on chat_messages), `07518d9` (deleted-message unread badge + bubble whitespace), `7a1573d`/`e2c3ae2` (sidebar last-message deletion fixes), `6c4396a`/`90cd5c8`/`1a05136`/`55a3cf3`/`c3bef4d` (delete tombstoning + optimistic delete + mention refresh), `e3f52c4` (Phase 1 keyboard-nav/responsive sidebar, Phase 3 lists/code/@mentions/draft, forward/reply/notif fixes), `41633ad` (copy/forward + media progress + echo dedupe + Tailwind `src/` fix + tile sizes + tray inset) · branch `master` (22 commits ahead of `origin/master`; uncommitted working-tree changes also present)
...
- **Phase 3 (≈100%)** — Lexical composer (bold/italic/underline), **lists/code toolbar** (`@lexical/list` unordered/ordered + `@lexical/code-core` `CodeNode`), **@mentions** (`MentionNode` DecoratorNode → `<span class="chat-mention">`, `MentionPlugin` @… dropdown with Arrow/Tab pick + commit), drag-&-drop + multi-file upload, voice notes, **full emoji picker** (1870 emojis, category nav, sticky headers — also used for reactions), **optimistic send-clear**, reply-quote bar, **staged attachment tray** with image editor (crop/tilt/draw), **draft persistence** (`chat-draft:{conversationId}` localStorage, restore via `$generateNodesFromDOM`, cleared on sent). **Text color + highlight swatches** added to composer toolbar (`ColorButtons.tsx`).
- **Phase 4 (≈100%)** — grouping, reactions (one per user), unified hover action menu (above/below positioning, **Copy/Forward/Reply/Edit/Delete**), inline edit with **5-line scrollable textarea + chevron formatting menu** (Bold/Italic/Underline/Strikethrough/Code + **text color swatches + highlight swatches**), **quote/reply fully wired** (composer quote bar → `reply_to_id` → bubble preview `Sender: snippet` + **image snippet thumbnail** → click-to-jump with highlight), **virtualized `<MessageList>`** (react-window v2 dynamic heights, per-conversation reset, scroll-to-latest / jump-to-row), **read-receipt avatars + WhatsApp ✓/✓✓ checkmarks**, default no-chat-selected panel + gray users-icon group avatars, **multi-recipient Forward modal with send confirmation**, **sidebar last-message previews** now show image thumbnails / file-type icons with text snippets instead of generic labels.
- **Phase 7 (partial ~55%)** — i18n layer (`en`/`es`, typed keys incl. `moreActions`, `replyingTo`, `typingMulti`, `copyMessage`, `forward*`, `forwardSelected`, `forwardNone`, `forwardDone`, `editorCrop`, `editorTilt`, `editorDraw`, emoji category names).
- **Production** — feature flag activated (`NEXT_PUBLIC_USE_NEW_CHAT=true`), Vercel deploy green, `GET_LOGS` route fixed, `.eslintcache` gitignored, action-menu refactor shipped, **Tailwind `src/` content fix** (oversized-collage root cause), **soft-delete tombstones + optimistic delete**, **REPLICA IDENTITY FULL** on `chat_messages`, **deleted-message unread badge fix**, **sidebar last-message deletion fixes**, **Lexical whitespace runtime fix**, **edit composer viewport-aware menu + color/highlight swatches**, **image editor integration**, **chat list file/image previews with text snippets**.

 **Active**
- Uncommitted working-tree changes: `ChatImageEditor.tsx`, `ColorButtons.tsx`, `Composer.tsx` text color/highlight integration, `AttachmentTray.tsx` image editor integration, `MessageList.tsx` edit menu + chat list previews, `ConversationListItem.tsx` previews, `chatService.ts` attachment fetch, `translations.ts` new keys, `utils.ts` helpers. Ready for commit + push.
- Confirm the full batch on screen (hard refresh): keyboard nav + mobile sidebar toggle (Phase 1), list/code/@mention toolbar + draft restore (Phase 3), multi-recipient Forward + send-confirmation, action-menu-internal scroll fix, reply image snippet, notification HTML strip, tray composer inset, image editor, edit formatting menu, chat list previews.
...
- `src/chat/components/ChatImageEditor.tsx` — image editing overlay (crop via `react-easy-crop`, tilt/rotate, draw pencil with color/size/undo/apply); integrated into `AttachmentTray` for staged images.
- `src/chat/components/AttachmentTray.tsx` — WhatsApp-style attachment preview tray (thumbnails/icon, chevron navigation, `n/N` counter, per-file remove, **image editor pencil button**, composer inset `pb-[max(1.28rem,6.4%)]`) — Phase 3.
...
`41633ad` (copy/forward + media-progress + echo dedupe + Tailwind `src/` fix + tile sizes + tray inset). Followed by: **Phase 1 keyboard-nav + responsive sidebar, Phase 3 lists/code/@mentions/draft, forward/reply/notif fixes `e3f52c4`**; **delete tombstoning + optimistic delete + mention refresh `c3bef4d`/`55a3cf3`/`1a05136`/`6c4396a`/`90cd5c8`**; **sidebar last-message deletion fixes `7a1573d`/`e2c3ae2`**; **deleted-message unread badge + bubble whitespace `07518d9`**; **REPLICA IDENTITY FULL on chat_messages `23a747a`**; **Lexical whitespace runtime fix `a3ace91`**. **Plus uncommitted working-tree changes**: `ChatImageEditor.tsx`, `ColorButtons.tsx`, `Composer.tsx` text color/highlight integration, `AttachmentTray.tsx` image editor integration, `MessageList.tsx` edit menu + chat list previews, `ConversationListItem.tsx` previews, `chatService.ts` attachment fetch, `translations.ts` new keys, `utils.ts` helpers.
