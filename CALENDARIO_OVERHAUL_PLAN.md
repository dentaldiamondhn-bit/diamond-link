...
> **Last updated:** 2026-09-09 · **Status:** Phases 0–3 complete, Phase 4 code complete + gate green (`██████████████████████ 80%`)
>
...
**Active**
- **Phase 4 — Custom RBC Rendering & DnD (🔨 2026-09-09, code written, gate green):**
  custom `components` — dental event pill (dentist color dot + patient + procedure badge),
  WhatsApp-style agenda rows with status chips, es toolbar (Hoy/‹/› + view switcher), es
  weekday/month headers; `withDragAndDrop` + `DndProvider` (`react-dnd` +
  `react-dnd-html5-backend` installed, addon CSS); `onEventDrop`/`onEventResize` →
  `calendarDnD.ts` (`dragTargetUpdates` incl. month/all-day date-only rebase, resize clamp,
  `findDentistOverlap` same-dentist chair/cubicle check that blocks the move with a toast);
  `updateEvent` mutation optimized (snapshot rollback). Files: `calendarComponents.tsx`,
  `calendarDnD.ts`, `RbcCalendar.tsx`, `CalendarShell.tsx`, `useCalendarData.ts`.
- **Invitee visibility (implemented 2026-09-09, code gate green):** user verified an event
  created by A now displays in invitee B's calendar. Fixes: `GET /api/events` returns
  **owned OR invited** events (`app/api/events/route.ts:26-48`, matches RLS + realtime
  `event_invitees` binding) — invitee calendars populate on SELECT, but realtime
  **move/resize** refresh for invitees is gated on **migration `20260909e`**
  (`event_invitees.updated_at`, NOT applied yet — PUT best-effort-touches it → fires B's
  SSE → invalidate). Non-owners: DnD blocked with toast (`No puedes mover una cita que no
  es tuya`), drawer hides Eliminar/Editar + "Solo el propietario…" note, DayDetail rows
  open the read-only drawer (no edit modal). Server hardening: `PUT` → 404 on PGRST116,
  `DELETE` → 404 when 0 rows matched (no more fake-success delete for non-owners).
  **No RSVP accept/decline exists** (`event_invitees.status` always `pending`; no PATCH
  endpoint, no bell — Phase 5). tsc 0 / ESLint 0 errors. Next: apply `20260909e` → verify
  live invitee realtime with two tabs.
...
### Next Move
1. Phase 4 tail: browser QA **C10** (drag → optimistic + server save + realtime peers),
   **C11** (resize end bound), **C12** (same-dentist collision blocked with message), plus a
   custom-rendering pass (pills/agenda rows/the badge dot) and a mobile milestone-wrap. On
   pass → mark Phase 4 ✅.
