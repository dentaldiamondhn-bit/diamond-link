...
> **Last updated:** 2026-09-08 · **Status:** Phases 0–2 complete, Phase 3 in progress (`████████████████░░░░░░ 80%`)
>
...
| **3** | Slot Selection & Event Modal UX | ◔ | `████████████████░░░░░░ 80%` | RBC slot closes create modal (un-pre-filled); event select pries open 883-line modal; no Zod/RHF; no drafts | **Implemented (2026-09-08):** `onSelectSlot({start,end})` + pre-filled create modal; `onSelectEvent` → detail drawer (edit/delete); **Zod + RHF first adoption** — multi-step Details→Timing→Invitees&Reminders with per-step validation; custom dentist/procedure (free-text escape) preserved; `calendar-draft:{date}` debounced autosave + one-tap restore; repo `set/getEventInvitees`/`set/getEventReminders` + `dateToTimeStr`; patient link → `/patient-preview/[id]`; modal + drawer hardened (Escape-to-close, `role=dialog`/`aria-modal`, initial focus), branch green (tsc 0 / ESLint 0); both legacy callers intact; gate green (tsc 0 / ESLint 0 errors / `next build` ✓) | QA C17–C21 + C25 pending (user role gate `tech_support` fixed — `calendarAuth.ts`); a11y focus trap (`useFocusTrap`) deferred to Phase 8; visual mobile pass |
...
| **3 – Slot Selection & Event Modal UX** | ◔ | `████████████░░░░░░░░░░ 60%` | `EventModal.tsx` (883 lines): patient search, invitee search (`/api/users`), `EVENT_COLORS` swatch, multi-reminder, delete; `TaskPanel` inline add/toggle/delete; no Zod/RHF | **Implemented (2026-09-08):** RBC callbacks wired — **`onSelectSlot={({start,end})}` opens the create modal pre-filled** (month/agenda fall back to clinic defaults; week/work_week/day carry the slot times) and **`onSelectEvent={({resource})}` opens a new `EventDetailDrawer`** (invitees/reminders loaded, patient deep-link `/patient-preview/[id]`, Edit → same modal, Delete → confirm). `EventModal` rebuild as modular **Zod + RHF** steps (*Details* patient search + procedure `PROCEDURES` + dentist `DENTISTS` → *Timing* pre-filled from slot → *Invitees & Reminders*), per-step `trigger` validation (C19 inline messages), `EVENT_COLORS` swatch, multi-reminders, delete-confirm; **custom dentist/procedure preserved** via free-text escape (C21 — `VARCHAR` columns, no enum). Draft persistence **`calendar-draft:{date}`** debounced autosave + one-tap Restaurar/Descartar (C20). New `src/calendario/event/` (`eventSchema.ts`, `eventDraft.ts`, `fields.tsx`); repo gained `setEventInvitees`/`setEventReminders`/`getEventInvitees`/`getEventReminders` + `rbcAdapter.dateToTimeStr`. Legacy props kept — `Dashboard` + `DayDetail` workflows untouched. Gate green: `tsc --noEmit` 0, scoped ESLint 0 errors (2 react-refresh warnings), `next build` ✓. **Remaining (Phase 3 tail):** QA **C17–C21 + C25** live; focus trap (`useFocusTrap`) deferred to Phase 8; visual mobile pass. |
...
**Active**
- **Phase 3 — Slot Selection & Event Modal UX (🔨 2026-09-08):** code complete +
  gate green (tsc 0 / ESLint 0 errors / `next build` ✓). `onSelectSlot({start,end})`
  pre-fills the create modal (week/work_week/day carry slot times; month/agenda default
  09:00–09:30); `onSelectEvent` opens a new `EventDetailDrawer` (invitees/reminders,
  patient deep-link `/patient-preview/[id]`, Edit → modal, Delete → confirm); 883-line
  modal split into **Zod + RHF** steps (Details → Timing → Invitees & Reminders),
  per-step validation, custom dentist/procedure free-text preserved, `calendar-draft:{date}`
  autosave + one-tap restore; repo gained invitee/reminder sync helpers; both legacy
  callers (`Dashboard`, `DayDetail`) intact. **Authz blocker cleared:** `tech_support`
  added to `CALENDAR_ROLES` (`src/lib/calendarAuth.ts`) + God-account fallback mirroring
  `middleware.ts` — calendar APIs are no longer 403 for the superuser account. Modal +
  drawer hardened: Escape-to-close (`role=dialog`/`aria-modal`, initial focus) so live QA
  (C17–C21, C25) can now run in-browser.
...
- Live QA (C01–C36) needs deployed env via real browser credentials (same constraint as the
  chat plan; in-session checks cover API/SW/server paths). Phase 3 tail = browser QA only.

### Next Move
1. Phase 3 tail: browser QA **C17–C21 + C25** (auth now passes for the superuser account);
   optional visual/mobile polish pass on the new modal + drawer.
2. On QA pass → mark Phase 3 ✅; Phase 4 (custom RBC event pills + agenda rows +
   `withDragAndDrop`) begins after.
