...
export async function GET(
  ...
  try {
    ...
    const { data: conversation, error: convError } = await supabase
      ...
      .select(`
        *,
        creator:users!chat_conversations_created_by_fkey(id, email, first_name, last_name, profile_image_url),
        participants:chat_participants(
          *,
          user:users(id, email, first_name, last_name, profile_image_url, role)
        )
      `)
      ...
      .select(`
        *,
        ...
        patient_case_link:chat_patient_case_links(
          *,
          patient:pacientes(id, nombre_completo)
        )
      `)
      .eq('conversation_id', conversationId)
      .eq('is_deleted', false)
