...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.log('Orthodontic delete request received:', { patientId, filePath, documentIndex });
    ...
    // First, try to delete from orthodontic storage
    console.log('Attempting to delete from orthodontic storage:', filePath);
    console.log('Storage bucket: orthodontic-documents');
    console.log('Admin client created:', !!adminClient);
    
    const { error: storageError, data: storageData } = await adminClient.storage
      .from('orthodontic-documents')
      .remove([filePath]);
    ...
    // Always update database to remove document URL
    const updatedDocuments = documents.filter((_: any, i: number) => i !== documentIndex);
    console.log('Updating orthodontic database with documents:', updatedDocuments);
    
    const { error: dbError, data: dbData } = await adminClient
      .from('historia_clinica_ortodoncia')
      .update({ documentos_ortodoncia: updatedDocuments })
    ...
    return NextResponse.json({
      success: true, 
      message: 'Orthodontic document deleted successfully',
