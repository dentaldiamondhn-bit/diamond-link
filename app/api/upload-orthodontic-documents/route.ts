...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get existing orthodontic history documents to check for duplicates
    ...
    // Upload non-duplicate files to orthodontic bucket
    const uploadedUrls = await StorageService.uploadOrthodonticDocuments(nonDuplicateFiles, patientId);
    ...
    const { error } = await supabaseAdmin
      .from('historia_clinica_ortodoncia')
      .update({ documentos_ortodoncia: allDocuments })
      .eq('paciente_id', patientId);

    if (error) {
      console.error('Error updating documents:', error);
      return NextResponse.json(
        { error: 'Failed to update documents: ' + error.message },
        { status: 500 }
      );
    }
...
async function checkFileExists(fileHash: string, existingUrls: string[]): Promise<boolean> {
  ...
  // In a production environment, you might want to store file hashes in database
