...
function PatientFollowUpPageContent() {
  ...
  const loadPatients = async () => {
    try {
      ...
      if (data.data) {
        ...
        console.log('Filtered patients count:', finalPatients.length);
        if (finalPatients.length > 0) {
          console.log('First 3 filtered patients:');
          finalPatients.slice(0, 3).forEach((p: PatientFollowUp, i: number) => {
            console.log(`${i + 1}. ${p.paciente_nombre} - ${p.dias_ultimo_tratamiento} days - ${p.fecha_ultimo_tratamiento}`);
          });
        }
  ...
  const handleWhatsAppClick = (patient: PatientFollowUp) => {
    if (patient.paciente_telefono) {
      const message = `Hola ${patient.paciente_nombre}, te recordamos que tu último tratamiento fue hace ${patient.dias_ultimo_tratamiento} días (${patient.ultimo_tratamiento}). ¿Te gustaría agendar una cita?`;
                  ...
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    ...
                    {patient.paciente_telefono && (
                      <button
                        ...
                        className="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-lg font-medium transition-colors self-start md:self-center"
                      >
