...
export default function ConsentimientoDocument() {
  ...
  const handleTemplateSelect = (templateId: string) => {
    ...
    if (template) {
      setConsentimientoContent(template.content);
    }
  };

  // Initialize with default template
  useEffect(() => {
    handleTemplateSelect('general');
  }, []);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const selectedTemplateData = getConsentTemplate(selectedTemplate);
      const consentimientoData = {
        paciente_id: pacienteId,
        tipo_consentimiento: selectedTemplateData?.type || 'otros',
        nombre_consentimiento: selectedTemplateData?.name || 'Consentimiento Informado',
        descripcion: selectedTemplateData?.description || 'Consentimiento informado para tratamientos odontológicos',
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 p-8 mb-8">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl p-6 mb-6 border border-gray-200 dark:border-gray-700">
            ...
            <select
              ...
              className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 dark:bg-gray-700 dark:text-white transition-colors duration-200"
            >
            ...
            <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
              Selecciona el tipo de consentimiento que deseas generar
            </p>
          </div>
            ...
            <div className="space-y-6 text-base leading-relaxed">
              ...
              <div 
                ...
                dangerouslySetInnerHTML={{
                  __html: (() => {
                    let content = consentimientoContent;
                    
                    // Replace patient info with styled spans
                    content = content.replace(
                      /\{\{PATIENT_NAME\}\}/g, 
                      `<span class="font-semibold border-b-2 border-gray-400 dark:border-gray-500 px-1 pb-1 inline-block">${patient?.nombre_completo || '_________________________'}</span>`
                    );
                    
                    content = content.replace(
                      /\{\{PATIENT_ID\}\}/g, 
                      `<span class="font-semibold border-b-2 border-gray-400 dark:border-gray-500 px-1 pb-1 inline-block">${patient?.numero_identidad || '_____________________'}</span>`
                    );
                    
                    content = content.replace(
                      /\{\{PATIENT_ADDRESS\}\}/g, 
                      `<span class="font-semibold border-b-2 border-gray-400 dark:border-gray-500 px-1 pb-1 inline-block">${patient?.direccion || '__________________________________________'}</span>`
                    );
                    
                    content = content.replace(
                      /\{\{DOCTOR_NAME\}\}/g, 
                      `<span class="font-semibold border-b-2 border-gray-400 dark:border-gray-500 px-1 pb-1 inline-block">${patient?.doctor || '_________________________'}</span>`
                    );
