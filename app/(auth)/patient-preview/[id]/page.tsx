...
export default function PatientPreviewPage() {
  ...
  const loadHistoricalModeSetting = async () => {
    try {
      ...
      if (globalResult.status === 'fulfilled' && globalResult.value.data) {
        ...
        console.log('✅ Global historical mode setting loaded:', { configValue: globalResult.value.data.config_value, globalBypass });
      }
      ...
      if (patientResult.status === 'fulfilled' && patientResult.value.data) {
        ...
        console.log('✅ Patient historical mode setting loaded:', { patientBypass, usingPatientSpecific: true });
        setBypassHistoricalMode(patientBypass);
      } else {
        console.log('ℹ️ No patient-specific setting found, using global setting');
  ...
  useEffect(() => {
    console.log('🔍 bypassHistoricalMode changed (patient-preview):', bypassHistoricalMode);
