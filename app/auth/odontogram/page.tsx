...
function OdontogramPageContent() {
  ...
  const loadPatientAndOdontogramData = async () => {
    try {
      ...
      setOdontogramasGuardados(history.map(h => ({
        ...
        fecha: h.odontograma.fecha_creacion,
  ...
  const getContadorEstados = () => {
    ...
    Object.values(dientesData).forEach(diente => {
      contador[diente.estado] = (contador[diente.estado] || 0) + 1;
    });
    ...
    <>
      ...
      <div className="mx-auto max-w-5xl px-5 mt-8">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg dark:shadow-xl border border-gray-200 dark:border-gray-700 p-6 flex flex-wrap gap-8 items-end">
          ...
          <div className="form-group flex-1 min-w-[220px]">
            <label className="block text-sm font-medium text-black dark:text-gray-100 mb-2">Fecha:</label>
            <input
              type="date"
              value={new Date().toISOString().split('T')[0]}
              disabled
              className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white disabled:bg-gray-100 dark:disabled:bg-gray-800 disabled:text-gray-500 dark:disabled:text-gray-400 dark:[color-scheme:dark]"
              style={{
                colorScheme: 'dark',
              }}
            />
