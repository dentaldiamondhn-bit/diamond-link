...
export class StorageService {
  ...
  static async uploadOrthodonticDocuments(
    files: File[], 
    patientId: string
  ): Promise<string[]> {
    try {
      console.log('=== ORTHODONTIC DOCUMENT UPLOAD DEBUG ===');
      console.log('Files to upload:', files.map(f => ({
        name: f.name,
        type: f.type,
        size: f.size
      ...
      console.error('Error in uploadOrthodonticDocuments:', error);
      throw error;
    }
  }

  static async deleteOrthodonticDocument(filePath: string): Promise<boolean> {
    try {
      ...
      if (error) {
        console.error('Error deleting orthodontic document:', error);
        return false;
      }

      return true;
    } catch (error) {
      console.error('Error in deleteOrthodonticDocument:', error);
      return false;
    }
  }

  static async getOrthodonticDocumentUrl(filePath: string): Promise<string | null> {
    try {
      const { data } = supabase.storage
        .from('orthodontic-documents')
        .getPublicUrl(filePath);
      ...
      console.error('Error getting orthodontic document URL:', error);
      return null;
    }
  }
