...
import { $createTextNode, $getSelection, $isRangeSelection, KEY_ARROW_DOWN_COMMAND, KEY_ARROW_UP_COMMAND, KEY_TAB_COMMAND, TextNode } from 'lexical';
import { useChatStore } from '@/chat/store/chatStore';
import { getUserDisplayName } from '@/chat/utils';
import { $createMentionNode } from '@/chat/mentionNode';
import type { ChatUser } from '@/types/chat';

/** Lightweight @mention: dropdown of users triggered by typing `@query`. */
export default function MentionPlugin() {
  ...
  const commit = useCallback(
    (user: ChatUser) => {
      editor.update(() => {
        ...
        sel.insertNodes([$createMentionNode(getUserDisplayName(user))]);
    ...
    return editor.registerUpdateListener(({ editorState }) => {
      ...
      editorState.read(() => {
        ...
        if (dom) {
          ...
          nextAnchor = { top: rect.top - 8, left: Math.min(rect.left, vw - 270) };
        } else {
  ...
  useEffect(() => {
    if (query === null || candidatesRef.current.length === 0) return;
