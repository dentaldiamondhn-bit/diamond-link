...
async function checkAndRunMigration() {
  try {
    console.log('🔍 Checking if orthodontic versioning tables exist...');
    ...
    const { data: tables, error: tableError } = await supabase
      ...
      .eq('table_name', 'historia_clinica_ortodoncia_versions');
...
CREATE INDEX IF NOT EXISTS idx_orthodoncia_versions_current ON historia_clinica_ortodoncia_versions(patient_id, is_current);
    ...
    if (tables && tables.length > 0) {
      console.log('✅ historia_clinica_ortodoncia_versions table exists');
      ...
      if (testError) {
        console.log('❌ Table exists but has issues:', testError.message);
      } else {
        console.log('✅ Database tables are ready for versioning!');
      }
    } else {
      console.log('❌ historia_clinica_ortodoncia_versions table does not exist');
      console.log('📝 Please run the migration manually in your Supabase dashboard');
    }
